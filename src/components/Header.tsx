...
const Header: React.FC<HeaderProps> = ({ userName, language, theme, translations, onSettingsClick, isSettingsOpen, onLanguageChange, onThemeChange }) => {
  return (
    <div className="relative">
      <header className={`flex flex-col md:flex-row items-center justify-between p-4 shadow-sm rounded-lg gap-4 md:gap-0 ${theme === 'dark' ? 'bg-gray-800' : 'bg-white'}`}>
        ...
        <div className="flex items-center justify-between w-full md:hidden">
          ...
          <div className="flex items-center space-x-2">
            <button className={`p-1 rounded-full transition-colors ${theme === 'dark' ? 'bg-gray-700 hover:bg-gray-600' : 'bg-gray-100 hover:bg-gray-200'}`}>
            ...
            <button onClick={onSettingsClick} className={`p-1 rounded-full transition-colors ${theme === 'dark' ? 'bg-gray-700 hover:bg-gray-600' : 'bg-gray-100 hover:bg-gray-200'}`}>
              <svg className="w-4 h-4 text-gray-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div className="flex flex-col items-center text-center md:flex-1">
          <h1 className={`text-lg md:text-xl font-semibold ${theme === 'dark' ? 'text-white' : 'text-gray-800'}`}>{translations.welcome}, {userName}</h1>
          <p className={`text-xs md:text-sm ${theme === 'dark' ? 'text-gray-300' : 'text-gray-600'}`}>{translations.monitoring}</p>
          <div className="hidden md:flex items-center mt-2 space-x-4">
